...
type EditorItem struct {
	...
	LastModified time.Time      `json:"last_modified"      validate:"required" example:"2023-01-15T10:30:00Z"`
}

type ScriptResult struct {
	StructuredResults map[string][]map[string]any `json:"structured_results,omitempty" example:"{\"customers.json\":[{\"name\":\"John\",\"age\":30}]}"` // Parsed resulting structured files
	HasErrors         bool                        `json:"has_errors,omitempty" example:"false"`
	Duration          time.Duration               `json:"duration,omitempty" example:"1s"`
	StartedAt         time.Time                   `json:"started_at,omitempty" example:"2023-01-15T10:30:00Z"`
	FinishedAt        time.Time                   `json:"finished_at,omitempty" example:"2023-01-15T10:30:00Z"`
	Logs              []string                    `json:"logs,omitempty"               validate:"dive" example:"[\"Log message 1\",\"Log message 2\"]"`
}
