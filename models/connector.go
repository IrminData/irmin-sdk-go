...
type Connector struct {
	...
	LogoURL string `json:"logo_url"          validate:"required,validimageurl" example:"https://cdn.irmin.dev/connectors/mysql-logo.png"`
	// Array of capabilities of the connector, eg. what kind of operations the connector can perform
	Capabilities []ConnectorCapability `json:"capabilities"      validate:"required,dive,oneof=pull push push_patch event_webhook" example:"[\"pull\",\"push\",\"event_webhook\"]"`
	...
	Categories []ConnectorCategory `json:"categories"        validate:"required,dive,oneof=database crm erp warehouse marketing analytics storage messaging payment social calendar project_management ecommerce iot monitoring other" example:"[\"database\",\"analytics\"]"`
